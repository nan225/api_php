...
<?php
$projets = json_decode(file_get_contents('http://localhost:8888/api_php/controllers/lireetudiant.php'));
...
if (isset($_POST['projets_id'])) {
    ...
    $data = array('id_etudiant' => $projetID);
    $result = CallAPI('DELETE', "deleteetudiant.php", $data);
...
<div  class="container-fluid">
    <table class="table" name="table">
        ...
        <tbody>
          <tr>
            <th scope="row"><?= $projet -> projet_id ?></th>
            <td><?= $projet -> id_etudiant ?></td>
            <td><?= $projet -> nom ?></td>
            <td><?= $projet -> prenoms ?></td>
            <td><?= $projet -> email ?></td>
            <td><?= $projet -> dateNaissance ?></td>
            <td>
            <?php
                        echo ' <form action="./vueetudiant.php" method="POST"><input type="hidden"
                          name="codeL" value=' . $projet->id_etudiant . ' >
